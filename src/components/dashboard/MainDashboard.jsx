...
const getGreeting = () => {
  ...
  if (hour < 18) return 'Bonne apres-midi';
  return 'Bonsoir';
};
...
const MainDashboard = ({ user, onNavigate, onLogout }) => {
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-slate-50 p-6 text-slate-800">
      <div className="z-10 w-full max-w-7xl px-3 sm:px-4 lg:px-5">
        <div className="w-full overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white px-4 py-4 shadow-sm sm:px-5">
            ...
            <div className="flex min-w-0 items-center gap-1.5 sm:gap-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 break-words text-sm font-medium text-slate-500">
                  {getGreeting()},{' '}
                  <span className="font-bold text-slate-800">{userLabel}</span>
                </div>
              </div>
            </div>
        ...
        <div className="mb-10 mt-4 flex w-full flex-col items-end gap-3 sm:flex-row sm:justify-end">
          <div className="flex items-center rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 text-[10px] font-bold text-emerald-800">
          ...
          {DASHBOARD_MODULES.map((module) => {
            ...
            return (
              <button
                ...
                className={`group relative rounded-3xl border-2 bg-white p-8 text-left shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl ${
                  allowed
              ...
              >
                <div
                  className={`mb-6 w-fit rounded-2xl p-4 ${
                    allowed
                      ? 'bg-blue-50 text-blue-900 transition-colors group-hover:bg-blue-900 group-hover:text-white'
                      : 'bg-slate-100 text-slate-400'
                  }`}
                >
                  <Icon size={32} />
                </div>
                <h2 className="mb-2 text-xl font-black text-slate-800 group-hover:text-blue-900">
                  {module.title}
                </h2>
      ...
      <div className="mt-12 text-center text-xs font-medium uppercase tracking-widest text-slate-400">
